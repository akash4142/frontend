...
const NewOrderButton = ({ fetchOrders }) => {
  ...
  const [newOrder, setNewOrder] = useState({
    ...
    customSupplier: "",
    expectedArrival: "",
    invoiceNumber: "", // ✅ Added Invoice Number Field
  });
  ...
  const handleCreateOrder = async () => {
    if (
      ...
      !newOrder.invoiceNumber // ✅ Ensure invoice number is entered
    ) {
      alert("❌ Please fill in all required fields, including Invoice Number!");
      return;
    }
    ...
    try {
      await createOrder({
        ...
        customSupplier: useCustomSupplier ? newOrder.customSupplier : null,
        expectedDelivery: newOrder.expectedArrival,
        invoiceNumber: newOrder.invoiceNumber, // ✅ Send Invoice Number
      });
